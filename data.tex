...
\end{align*}
と算出できる．ただし$\yj{\hat y}$はYeo-Johnson変換後の値として意味を持つので，直接解釈は出来ない．そのため，リスクマップを構成する際には，$\yj{\hat y}$へYeo-Johnson逆変換を施し，$\hat y$とする．
%\yj{y}_i
...
PAI\cite{chainey2008utility}に加え，分類学習で一般的なarea under the receiver operating characteristic（ROC） curve\cite{islp}で年単位の評価を行う．
...
これらのスケールパラメータの値はシカゴ市の2ブロック，4ブロック，6ブロック，8ブロックに相当する．
...
%------------------------------------
学習，検証，テストに関わるデータの分割は以下の通りである．
1年ずつ予測変数と応答変数の対をずらすことで，予測に未来の情報を用いないようにする．
...
Lasso回帰の正則化パラメータ$\nu$の最適化は，20分割の交差検証\cite{islp}で実施した．
